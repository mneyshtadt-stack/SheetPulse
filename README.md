...
The combined chart's pre-open stretch (TASE trading alone, before USA opens) handles this differently: rather than holding USA flat at yesterday's ILS figure, it re-prices `USA market$` (yesterday's USD close) at **the actual USD/ILS rate as of the start of today's TASE session (~09:59)**, so that segment reflects real FX movement even before USA opens. That rate is captured once by the Apps Script, a couple of minutes before TASE opens, and stored as a 3rd column on `IntradayLogIL`'s first row of the day (`USD/ILS Rate (session open)`) — a real, fixed historical fact rather than whatever rate happens to be live at whatever later moment someone views the chart, so it doesn't drift on every refresh. That stretch is marked with a solid accent-colored dot and a "~" tag at its start and end, and hovering it shows a tooltip with both rates side by side (session-open vs. the previous close's own rate) so the FX-driven portion of the change is visible at a glance. This only affects the chart's plotted line — the `Prev. close`/`Day Change` header figures stay anchored to the real historical close on purpose, so they never disagree with their own label. The transition itself is explicitly anchored at USA's nominal open (16:30 Israel time) rather than wherever TASE's own ~5-minute logging cadence or USA's actual first real row happen to land, once that moment has passed — so the estimated segment always has a real plotted point at that precise instant, not a few minutes early or late.
