...
When today's own trading window has fewer than 2 logged points yet (before that market opens, or a weekend/holiday), each intraday chart (IL, USA, Portfolio) falls back to replaying the most recently completed session instead of showing an empty chart — walking backward, day by day, through the distinct calendar dates actually present in `IntradayLog` (which keeps a rolling **2-day** window for exactly this reason) until it finds one with real data. That replayed session shows its *own* correct `Prev. close` — e.g. Tuesday's close when replaying Monday, not today's not-yet-relevant one — by looking up `LastClose`'s full per-date history (`window.lastCloseByDate`, built from every dated row in the 7-row `LastClose` sheet, not just the single most recent value) for the closest recorded close strictly before the replayed date.
