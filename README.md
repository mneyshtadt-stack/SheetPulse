...
Intraday price history (used for the IL/US/combined intraday charts) is logged separately: an Apps Script time-driven trigger bound to the spreadsheet snapshots the portfolio's value once a minute into `IntradayLogIL`/`IntradayLogUS` tabs while each market is open, stamping each row's Timestamp column as `dd/mm/yyyy hh:mm:ss`. The dashboard parses that column with an explicit dd/mm/yyyy parser rather than JavaScript's native `Date` constructor, since the native parser only reliably understands `m/d/yyyy` order for slash-separated strings. `IntradayLogIL` also carries a 3rd column, `USD/ILS Rate (session open)`, populated on exactly two rows a day — its own first row (TASE's ~09:58 open) and whichever row happens to log during the ~16:28–16:30 window (USA's own open; TASE's trigger is still running then, so this piggybacks on the regular per-minute write rather than needing separate logic in `IntradayLogUS`, which never carries this column at all). Each row's own `Timestamp` is what tells the two captures apart, not a separate column — see below.
...
The combined chart's pre-open stretch (TASE trading alone, before USA opens) handles this differently: rather than holding USA flat at yesterday's ILS figure, it re-prices `USA market$` (yesterday's USD close) at a real captured USD/ILS rate, so that segment reflects real FX movement even before USA opens. Two rates get captured through the day — TASE's own session-open (~09:59) and USA's (~16:29) — and the chart always prefers the fresher one: USA's ~16:29 capture once it exists (the more accurate figure as the day gets closer to USA's open), falling back to TASE's ~09:59 one earlier in the morning. Either way it's a real, fixed historical fact rather than whatever rate happens to be live at whatever later moment someone views the chart, so it doesn't drift on every refresh. That stretch is marked with a solid accent-colored dot and a "~" tag at its start and end, and hovering it shows a tooltip with both rates side by side (whichever capture was used vs. the previous close's own rate) so the FX-driven portion of the change is visible at a glance. This only affects the chart's plotted line — the `Prev. close`/`Day Change` header figures stay anchored to the real historical close on purpose, so they never disagree with their own label. The transition itself is explicitly anchored at USA's nominal open (16:30 Israel time) rather than wherever TASE's own ~5-minute logging cadence or USA's actual first real row happen to land, once that moment has passed — so the estimated segment always has a real plotted point at that precise instant, not a few minutes early or late.
